...
export default function CardUser({ isOnModal, onOpen, item, setItem, deleteUser, isDeleting, navigation, restoreUser }) {

    async function confirm(id, type) {
        Alert.alert(
            'Confirmação',
            type === 'delete' ? 'Realmente deseja excluir esse usuário?' : 'Realmente deseja restaurar esse usuário?',
            [
                {text: 'Sim', onPress: () => type === 'delete' ? deleteUser(id) : restoreUser(id)},
                ...
                <>
                    {(item.status === 'Ativo') ? (
                        <View style={styles.buttonsGroup}>
                            <TouchableOpacity onPress={() => navigation.push('EditUsers', { user: item })} style={styles.row}>
