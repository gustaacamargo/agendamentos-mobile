...
export default function CardCategory({ isOnModal, onOpen, item, setItem, deleteCategory, isDeleting, navigation, restoreCategory }) {

    async function confirm(id, type) {
        Alert.alert(
            'Confirmação',
            type === 'delete' ? 'Realmente deseja excluir esse ano?' : 'Realmente deseja reativar esse ano?',
            [
                {text: 'Sim', onPress: () => type === 'delete' ? deleteCategory(id) : restoreCategory(id)},
